...
def check_events(ai_settings,screen,ship,bullets,stats,play_button,aliens,sb):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_play_button(stats, play_button, mouse_x, mouse_y,aliens,bullets,screen,ship,ai_settings,sb)
def update_score(stats):
    """更新最高得分"""
    filename = 'score.txt'
    highscore = stats.high_score
    with open (filename) as file_object: 
        last_record = file_object.read()        
    if highscore > int(last_record):
...
def check_play_button(stats,play_button,mouse_x,mouse_y,aliens,bullets,screen,ship,ai_settings,sb):
    """在玩家单击Play按钮时开始新游戏"""
    button_cliked = play_button.rect.collidepoint(mouse_x,mouse_y)
    if button_cliked and  not stats.game_avtive:
        start_game(stats,aliens,bullets,ai_settings,screen,ship,sb)
